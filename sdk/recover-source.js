...
const argv = yargs(hideBin(process.argv))
    ...
    .option('input', {
        description:
            'The input path to the minified JavaScript file or a directory containing multiple files',
        alias: 'i',
        type: 'string',
        demandOption: true,
    })
    ...
    await SourceMapConsumer.with(
        ...
        (sourceMapConsumer) => {
            ...
            const sourceContents = sources.reduce((contents, source) => {
                contents[source] = sourceMapConsumer.sourceContentFor(
                    source,
                    true
                );
                return contents;
            ...
            if (
                Object.values(sourceContents).some(
                ...
                for (const source in sourceContents) {
                    if (sourceContents[source] !== null) {
                        ...
                        fs.mkdirSync(path.dirname(originalFilePath), {
                            recursive: true,
                        });
                        fs.writeFileSync(
                            originalFilePath,
                        ...
                        console.log(
                            `Source code recovered to ${originalFilePath}`
                        );
                    }
                }
            } else {
                ...
                lines.forEach((line, lineIndex) => {
                    ...
                    for (let column = 0; column < columnCount; column++) {
                        ...
                        const originalPosition =
                            sourceMapConsumer.originalPositionFor(pos);
                ...
                // prettify the code
                try {
                    reconstructedSource = prettier.format(reconstructedSource, {
                        semi: false,
                        parser: 'babel',
                    });
                } catch (error) {
                    console.error(
                        'An error occurred while prettifying the code:',
                        error
                    );
                }
                ...
                const originalFilePath = path.join(
                    path.dirname(minifiedFilePath),
                    path.basename(minifiedFilePath, '.js') + '-recovered.js'
                );
                fs.mkdirSync(path.dirname(originalFilePath), {
                    recursive: true,
                });
